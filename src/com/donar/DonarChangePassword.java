 ...
 */
@WebServlet("/DonarChangePassword")
public class DonarChangePassword extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet resultset = statement.executeQuery("select Password from tblblooddonars where Password='" + cpassword+ "' and FullName ='" + session.getAttribute("FullName") + "'");
			if (resultset.next()) {
				pass = resultset.getString(1);
			}
			if (password.equals(confpass)) {
				if (pass.equals(cpassword)) {
					int i = statement.executeUpdate("update tblblooddonars set Password='" + password + "' where FullName ='" + session.getAttribute("FullName") + "'");
					String message = "Password change successfully.";
					session.setAttribute("password-change-success", message);
					response.sendRedirect("donar-change-password.jsp");
				} else {
